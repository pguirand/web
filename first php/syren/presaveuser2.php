		...
		*/
		...
		if (isset($_GET['NUM_NIF'])) {
		  $colname_rechByNif = $_GET['NUM_NIF'];
			...
			$query_rechByNif = sprintf("SELECT * FROM individu, DOCUMENT WHERE ".$champs." = %s AND INDIVIDU.ID_IND=DOCUMENT.ID_IND ORDER BY ".$champs." ASC", GetSQLValueString($colname_rechByNif, "text"));
...
No Passeport</label>
                        <br />
                        <input type="radio" name="choixId" value="NUM_DOC" <?php if($_GET['choixId'] == "NUM_PASSPORT") echo "checked='checked'" ?>  id="RadioGroup1_3" />
                      No Document </td>
                    </tr>
                    <tr>
...
?>
  <div align="center">
   ...
   <div class="fonce">Résultats</div> 
    ...
    <br><br>
        <a href="creation_usr.php?ID_IND=<?php echo $row_rechByNif['ID_IND']."&NOM_IND=".$row_rechByNif['NOM_IND'] ?>">Cr&eacute;er l'utilisateur</a>  </div>
  <?php /*?><?php
