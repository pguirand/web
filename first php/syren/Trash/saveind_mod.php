...
if (isset($_SERVER['QUERY_STRING'])) {
  ...
  $IDARR = $_POST['ARRONDISSEMENT'];
  $IDCOM = $_POST['COMMUNE'];
  $IDSECCOM = $_POST['SECCOM'];
...
mysql_select_db($database_connex, $connex);
$query_COMMUNE = "SELECT commune.ID_COM, commune.NOM_COM FROM commune, arrondissement WHERE commune.ID_ARRON = arrondissement.ID_ARRON";
...
mysql_select_db($database_connex, $connex);
$query_SECCOM = "SELECT section_com.ID_SECCOM, section_com.NOM_SECCOM FROM section_com, commune WHERE section_com.ID_COM = commune.ID_COM";
...
mysql_select_db($database_connex, $connex);
$query_ARRONDISSEMENT = "SELECT arrondissement.ID_ARRON, arrondissement.NOM_ARRON FROM arrondissement, departement WHERE arrondissement.ID_DEPT = departement.ID_DEPT";
...
echo $connexion = mysql_pconnect($database_connex, $connex);
echo $connexion == true;
if($connexion != false)

echo    $choixbase = mysql_select_db($database_connex, $connex);
echo    $sql1 = "SELECT `ID_DEPT`, `NOM_DEPT`".
    " FROM `DEPARTEMENT`".
    " ORDER BY `ID_DEPT`";
    $rech_dep = mysql_query($sql1);
    $code_dep = array();
    $dep = array();
    ...
    if($rech_dep != false)
    {
        while($ligne = mysql_fetch_assoc($rech_dep))
        {
            array_push($code_dep, $ligne['ID_DEPT']);
            array_push($dep, $ligne['NOM_DEPT']);
    ...
    ?>
<form action="<?php echo($_SERVER['PHP_SELF']); ?>" method="post" id="chgarr">
<fieldset style="border: 3px double #333399">
<legend>Sélectionnez un dept</legend>
<select name="dept" id="dept" onchange="document.forms['chgarr'].submit();">
  <option value="-1">- - - Choisissez un arr - - -</option>
    <?php
    for($i = 0; $i < $nb_dep; $i++)
    {
?>
  <option value="<?php echo($code_dep[$i]); ?>"<?php echo((isset($idr) && $idr == $code_dep[$i])?" selected=\"selected\"":null); ?>><?php echo($dep[$i]); ?></option>
<?php
    }
    ?>
</select>
    ...
    if(isset($idr) && $idr != -1)
    {
        ...
        if($connexion != false)
        {
            $rech_arr = mysql_query($sql2, $connexion);
            ...
            while($ligne_arr = mysql_fetch_assoc($rech_arr))
            {
                array_push($code_arr, $ligne_arr['ID_ARRON']);
                array_push($nom_arr, $ligne_arr['NOM_ARRON']);
                $na++;
            }
            /* Maintenant on peut construire la liste déroulante */
            ?>
<select name="arrondissment" id="arrondissment">
            <?php  
            for($d = 0; $d<$na; $d++)
            {
                ?>
  <option value="<?php echo($code_arr[$d]); ?>"<?php echo((isset($arr_selectionne) && $arr_selectionne == $code_arr[$d])?" selected=\"selected\"":null); ?>><?php echo($nom_arr[$d]." (". $code_arr[$d] .")"); ?></option>
                <?php
            }
?>
</select>
...
<p>Un incident s'est produit lors de la connexion à la base de données, veuiillez essayer à nouveau ultérieurement.</p>
        ...
        <?php
			// LOCALISATION DE L'ENREGISTREM
			$localisationDone == "non";

		?>
        			<form action="" method="post" id="chgarr">
                    <table>
                    <tr>
                      <td>ADRESSE</td>
                      <td><p align="left">
                        <select name="DEPARTEMENT" id="DEPARTEMENT" onchange="document.forms['chgarr'].submit();">
...
?>
                        ...
                        <select name="ARRONDISSEMENT" id="ARRONDISSEMENT">
                          ...
                          <?php
do {  
?><option value="<?php echo $row_ARRONDISSEMENT['ID_ARRON']?>"><?php echo $row_ARRONDISSEMENT['NOM_ARRON']?></option>
<?php
...
<br />
               ...
               <?php
			   // SI LOCALISATION FINI (= OUI), AFFICHER LA FORME COMPLETE D'ENREGISTREMENT
			   if ($localisationDone == "oui")
			   {
			   ?>
                <form id="chgarr" name="form2" method="POST" action="<?php echo $editFormAction; ?>">
...
<br />
                    ...
                    -->
                    <tr>
                      <td>TYPE IDENTIFIANT PERE</td>
                      <td><label>
                        <select name="TYPE_IDENTIFIANT_PERE" size="1" id="TYPE_IDENTIFIANT_PERE">
                          <option selected="selected">NUMERO NIF</option>
                    ...
                    <tr>
                      <td>TYPE IDENTIFIANT OFFICIER</td>
                      <td><select name="TYPE_IDENTIFIANT_OEC" size="1" id="TYPE_IDENTIFIANT_OEC">
                          <option selected="selected">NUMERO NIF</option>
                          <option>NUMERO CIF</option>
                          <option>NUMERO PASSEPORT</option>
                      </select></td>
                    ...
                    <tr>
                      <td>TYPE IDENTIFIANT TEMOIN 1</td>
                      <td><label></label>
                        <select name="TYPE_IDENTIFICATION_TEMOIN1" size="1" id="TYPE_IDENTIFICATION_TEMOIN1">
                          <option selected="selected">NUMERO NIF</option>
                          <option>NUMERO CIF</option>
                          <option>NUMERO PASSEPORT</option>
                        </select></td>
                    ...
                    <tr>
                      <td>TYPE IDENTIFIANT TEMOIN 2</td>
                      <td><label>
                        <select name="TYPE_IDENTIFICATION_TEMOIN2" size="1" id="TYPE_IDENTIFICATION_TEMOIN2">
                          <option selected="selected">NUMERO NIF</option>
                          <option>NUMERO CIF</option>
                          <option>NUMERO PASSEPORT</option>
                        </select>
