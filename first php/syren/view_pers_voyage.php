...
$colname_id_voyage = "-1";
if (isset($_GET['NO_VOL'])) {
  $colname_id_voyage = $_GET['NO_VOL'];
}
mysql_select_db($database_connex, $connex);
$query_id_voyage = sprintf("SELECT * FROM voyage WHERE NO_VOL = %s", GetSQLValueString($colname_id_voyage, "text"));
$id_voyage = mysql_query($query_id_voyage, $connex) or die(mysql_error());
$row_id_voyage = mysql_fetch_assoc($id_voyage);
$totalRows_id_voyage = mysql_num_rows($id_voyage);

$colname_listepers = "-1";
if (isset($_GET['NO_VOL'])) {
  $colname_listepers = $_GET['NO_VOL'];
}
mysql_select_db($database_connex, $connex);
$query_listepers = sprintf("SELECT v.NOM_COMP_AERIENNE, v.NO_VOL, i.NOM_IND, i.PRENOM_IND, i.NUM_PASSPORT, i.SEXE_IND FROM voyage v, individu i WHERE V.NO_VOL = %s AND V.ID_IND = I.ID_IND ORDER BY NO_VOL ASC", GetSQLValueString($colname_listepers, "text"));
$listepers = mysql_query($query_listepers, $connex) or die(mysql_error());
$row_listepers = mysql_fetch_assoc($listepers);
$totalRows_listepers = mysql_num_rows($listepers);
...
?>
  ...
  <div class="content"> 
    <p align="center"><span class="fonce style16"><em>Immigration et Emigration</em></span><br />
      <br />
    </p>
    ...
    <table border="1" id="comliste">
      ...
      <?php do { ?>
          <tr>
            <td><?php echo $row_listepers['NOM_IND']; ?></td>
            <td><?php echo $row_listepers['PRENOM_IND']; ?></td>
            <td><?php echo $row_listepers['SEXE_IND']; ?></td>
            <td><?php echo $row_listepers['NUM_PASSPORT']; ?></td>
          </tr>
          <?php } while ($row_listepers = mysql_fetch_assoc($listepers)); ?>
    </table>
    <p align="center"><br />
      <br />
    </p>
...
<?php
mysql_free_result($id_voyage);

mysql_free_result($listepers);
