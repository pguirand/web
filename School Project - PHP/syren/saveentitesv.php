<?php require_once('Connections/connex.php'); ?><?php require_once('Connections/connex.php');
...
function checkform()
                    {
	                 ...
	                 if(eMail1.value.match(emailExp)){
		                  ...
		                  alert('Format eMail incorrect: jonh@doe.com');
		                  return false;
	                     }
                     }
</script>
...
// Processus de renouvellement des valeurs au cas ou les valeurs du champs ID_ENTITE existerait deja

	$value_exist=verify_value("syren","entite","ID_ENTITE",$IDENT);
	
	do
		{
		$value_exist=verify_value("syren","entite","ID_ENTITE",$IDENT);
		$IDENT = rand(000000000000,999999999999);
		}
	while ($value_exist == 1);
	
//echo $IDENT = $name.$dateb.$pat.$totalRows_IDENTITE;
$target_path = "images/photos/entites/";
$target_path = $target_path.basename($_FILES['logo']['name']);
$type_file = $_FILES['fichier']['type'];


echo $insertSQL = sprintf("INSERT INTO entite (ID_ENTITE, NOM_ENTITE, NUM_PATENTE, SIGLE, SECTEUR_ACTIVITE, STATUT, NIVEAU, EMAIL1, EMAIL2, EMAIL3, SITE_WEB, LOGO, CREER_PAR) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
?>
            ...
            <div align="center" class="title style16"> 
		      ...
		      <div class="story">
		        ...
		        <form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form2" id="form2" onSubmit="return checkform()">  
				  ...
				  <input type="hidden" maxlength="20" name="ident" value="<?php echo rand(00000000000,9999999999999) ?>" />
		        </form>
