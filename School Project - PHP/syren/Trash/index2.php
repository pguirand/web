...
if (isset($_POST['NOM_UTIL'])) {
  ...
  if ($loginFoundUser) {
	...
	#	Trouver le groupe de l'internaute logge
		$userGroup	=	$row_loggedUser['groupe_id'];
		$getGroup = "SELECT * FROM utilisateur,groupe WHERE utilisateur.groupe_id = '".$userGroup."' AND utilisateur.groupe_id = groupe.groupe_id";
//die();		
...
switch($groupe)
	{
		case "administrateur"	:		$MM_redirectLoginSuccess = "admininterf.php";
									break;
		case "utilisateur"	:		$MM_redirectLoginSuccess = "user.php";
									break;
		case "userdefined"	:		// faire la requete pour trouver l'entite du user logge, pour retrouver la variable $entite
									switch($entite)
										{
											case "dgi"	:	$MM_redirectLoginSuccess = "dgi.php";
										}									
									break;									
		//default			:		$MM_redirectLoginSuccess = "admininterf.php";
	}
//	die();
