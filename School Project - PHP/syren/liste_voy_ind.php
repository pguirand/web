...
$maxRows_liste_voyages = 10;
$pageNum_liste_voyages = 0;
if (isset($_GET['pageNum_liste_voyages'])) {
  $pageNum_liste_voyages = $_GET['pageNum_liste_voyages'];
}
$startRow_liste_voyages = $pageNum_liste_voyages * $maxRows_liste_voyages;

$colname_liste_voyages = "-1";
if (isset($_GET['ID_IND'])) {
  $colname_liste_voyages = $_GET['ID_IND'];
}
mysql_select_db($database_connex, $connex);
$query_liste_voyages = sprintf("SELECT * FROM voyage WHERE ID_IND = %s", GetSQLValueString($colname_liste_voyages, "text"));
$query_limit_liste_voyages = sprintf("%s LIMIT %d, %d", $query_liste_voyages, $startRow_liste_voyages, $maxRows_liste_voyages);
$liste_voyages = mysql_query($query_limit_liste_voyages, $connex) or die(mysql_error());
$row_liste_voyages = mysql_fetch_assoc($liste_voyages);

if (isset($_GET['totalRows_liste_voyages'])) {
  $totalRows_liste_voyages = $_GET['totalRows_liste_voyages'];
} else {
  $all_liste_voyages = mysql_query($query_liste_voyages);
  $totalRows_liste_voyages = mysql_num_rows($all_liste_voyages);
}
$totalPages_liste_voyages = ceil($totalRows_liste_voyages/$maxRows_liste_voyages)-1;
?>
...
?>
  ...
  <table border="1" id="comliste">
    <tr class="head">
      <td>No</td>
      <td>NOM_COMP_AERIENNE</td>
      <td>PROVENANCE_VOL</td>
      <td>DESTINATION_VOL</td>
    </tr>
    <?php do { ?>
        <tr>
       	  <td> <?php echo $i=$i+1; ?></td>
          <td><?php echo $row_liste_voyages['NOM_COMP_AERIENNE']; ?></td>
          <td><?php echo $row_liste_voyages['PROVENANCE_VOL']; ?></td>
          <td><?php echo $row_liste_voyages['DESTINATION_VOL']; ?></td>
        </tr>
        <?php } while ($row_liste_voyages = mysql_fetch_assoc($liste_voyages)); ?>
  </table>
  </p><p>&nbsp;              
    </p><p>
      <br /><br /><br />
...
<?php
mysql_free_result($liste_voyages);
?>
